...
def setup_mcp(app: FastAPI) -> FastApiMCP:
    ...
    # Note: the bulk get_* read tools (get_items, get_list, get_lists) are kept
    # available for simplicity — clients expect them. They return all items
    # with no pagination, so on an unusually large list they can overflow a
    # client's per-tool-result token limit; for those cases use query_sql,
    # whose description carries the full schema (see efficient-queries skill).
